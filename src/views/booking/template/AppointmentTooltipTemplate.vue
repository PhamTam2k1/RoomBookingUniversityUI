    ...
    <div class="info">
      <div
        ...
        v-if="
          ...
          templateTooltipModel.StatusBooking == Enum.BookingStatus.Pending
        "
      >
...
export default {
  ...
  mounted() {
    const momentObj = moment(
      this.templateTooltipModel.StartDate,
    )
    this.dateBooking = momentObj.format('DD/MM/YYYY')
    this.roomChoose = this.dataRoom.find(
      (x) => x.RoomID == this.templateTooltipModel.RoomID,
    )
    this.isNotAdminDefult =
      this.templateTooltipModel.FullName == 'Admin' ? true : false
  },
}
</script>
