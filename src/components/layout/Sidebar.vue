<template>
  <aside :class="`${is_expanded ? 'is-expanded' : ''}`">
    <div class="logo" :class="`${is_expanded ? 'flex-expand' : ''}`">
	  ...
	  <div :class="`${is_expanded ? 'text-expand' : 'hiding-text'}`">Hệ thống đặt lịch phòng học</div>
    </div>

    <div class="menu">
        <base-sibar-item  v-for="item in MenuItems" :key="item.icon"
          :itemIcon="item.icon"
    ...
    <div class="menu">
      ...
      <button class="menu-toggle" @click="ToggleMenu">
          <font-awesome-icon icon="fa-solid fa-square-caret-right" style="color: #f8f9fc;" size="2xl"/>
      </button>
    </div>
    </div>
  </aside>
